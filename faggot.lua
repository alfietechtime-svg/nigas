...
-- ============================================================
-- PLAYER GUI LOADER (WAITS FOR FULLY LOADED)
-- ============================================================
local PlayerGui
local function waitForPlayerGui()
    ...
    if not PlayerGui then
        print("[PlayerGui] PlayerGui not found, creating one...")
    ...
    while not isReady and waitAttempts < 30 do
        ...
        isReady = PlayerGui and PlayerGui.Parent ~= nil
        if not isReady then
            task.wait(0.1)
        end
    end
    ...
    if PlayerGui and not PlayerGui:FindFirstChild("_ReadyCheck") then
        ...
        check.Parent = PlayerGui
        task.wait()
        check:Destroy()
    end
    
    print("[PlayerGui] PlayerGui is ready!")
    return PlayerGui
end
